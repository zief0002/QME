...
oz = analyze(math, math_key) # NOTE: should include test name in data structure for reporting

```

```{r functions_def, echo = FALSE}

## All these functions take the output of analyze as an input
...
basicdf$item = gsub("item", "", rownames(basicdf))
...
ggplot(basicdf, aes(x = difficulty)) + 
  geom_dotplot() + 
...
scores = data.frame(scores = oz$test_level$descriptives$scores)
